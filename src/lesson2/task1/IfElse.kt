...
fun timeForHalfWay(t1: Double, v1: Double,
    ...
    return when {
        half < s1 -> half/v1
        half in s1..(s1 + s2) -> if (v2 != 0.0) t1 + (half-s1) / v2 else t1 + (half - s1)
        v3 > 0.0 -> t1 + t2 + (half - s1 -s2) / v3
        else -> t1 + t2 + (half - s1 - s2)
    }
}
...
fun rookOrBishopThreatens(kingX: Int, kingY: Int,
    ...
    val distanceX = Math.abs(kingX - bishopX)
    val distanceY = Math.abs(kingY - bishopY)
    if ((kingX == rookX || kingY == rookY) && (distanceX != distanceY))
        return 1
    else if ((distanceX == distanceY) && (kingX != rookX && kingY != rookY))
        return 2
    else if ((distanceX == distanceY) && (kingX == rookX || kingY == rookY))
        return 3
    else
